...
-(void)fetchdata{
    ...
    else
    {
        ...
        dispatch_async(q_background, ^{
            ...
            NSString *post =[NSString stringWithFormat:@"useraccesstoken=%@&recipesubcategoryid=%@&devicetype=%@",atoken,_recipecategoryid,@"1"];
            NSData *postData = [post dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
            NSString *postLength = [NSString stringWithFormat:@"%d", [postData length]];
            NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
            NSString *urlString = [NSString stringWithFormat:@"%@getrecipelist", purl];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    for (int i=0; i<newDataArray.count; i++) {
                        ...
                        [topBar addSubview:umv];
                         //  });
                                               ...
                                               AsyncImageView *img=[[AsyncImageView alloc]initWithFrame:CGRectMake(5, 2, 35, 35)];
                                                img.imageURL=[NSURL URLWithString:[NSString stringWithFormat:@"%@",[newDataArray[i] objectForKey:@"image"]]];
                        ...
                        
                        {
                            ...
                            alphatext.text=[newDataArray[i] objectForKey:@"recipe_name"];
...
//    rvc.recipecategoryid=_recipecategoryid;
//    rvc.recipecategoryName=_recipecategoryName;
...
-(void)showdata:(NSString *)query{
    ...
    for (int i=0; i<newDataArray.count; i++) {
        ...
        if(im==1){
            ...
            substringRange= [[[newDataArray[i] objectForKey:@"recipe_name"] lowercaseString] rangeOfString:[query lowercaseString]];
...
-(void)categoryClick:(id)Sender{
    ...
    RecipeDetailsViewController *rvc=[[RecipeDetailsViewController alloc]init];
    rvc.recipeid=[NSString stringWithFormat:@"%ld",(long)[Sender tag]];
    rvc.recipesubcategoryid=_recipecategoryid;
     [self.navigationController pushViewController:rvc animated:YES];
